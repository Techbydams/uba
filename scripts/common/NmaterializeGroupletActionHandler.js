...
handleResetButtons = function(params) {
    ...
    for (i = 0; i < resetButtons.length; i++) {
        ...
        if ((buttonName == 'ACTION.RESET' || buttonName == 'ACTION.CLEAR') &&
            (jQuery(resetButton).parent().hasClass("clearButtonStyle") == true ||
                jQuery(resetButton).hasClass("clearButtonStyle") == true)) {
            ...
            if (!params.isMultiPartRequest) {
                ...
                if (isDownloadButton === Constants.TRUE) {
                    ...
                    if (feba.domManipulator.getElementById("MODAL_VIEW_CONTAINER").length > 0) {
                        ...
                        var textElements = feba.domManipulator.find(feba.domManipulator.getElementById("MODAL_VIEW_CONTAINER"),
                            getSpecifiedElements(params.groupletId.toUpperCase(), feba.domManipulator.getElement(encryptionRequired = true).text()));
                    } else {
                        ...
                        var textElements = getSpecifiedElements(params.groupletId.toUpperCase(), feba.domManipulator.getElement(encryptionRequired = true).text());
                    }
...
addFormAttr = function(frm, frameName, action) {
    ...
    if (((jQuery.browser.safari) || (jQuery.browser.chrome)) && (navigator.userAgent.match(/(iPad|iPhone|iPod)/g)) && navigator.userAgent.match(/like Mac OS X/i)) {
