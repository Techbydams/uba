...
jQuery(document).ready(function() {
    ...
    // Changes for TOL 791142
    writeCookie('bankId');
    //Added for RTL , JS Update for Styles
...
function sessionTimer(cssURL1, usertype) {
    ...
    var continueSessionButton = getMessage("ContinueSessionButton");
    //Added by Parvathy for UBA
    var Close = getMessage("Close");
    var logOutSessionButton = getMessage("LogOutSessionButton");
    /*
    ...
    this.createSessionPopupDoc = function() {
        ...
        var sessionLogOutURL = document.getElementById('sessionLogOutURL').value;
        ...
        //Added for RTL LTR changes
        var langId = "L001";
        if (jQuery("#languageId").length > 0) {
            langId = "L" + jQuery("#languageId").val();
        }
        var userTypeName = "consumer";
        if (usertype == 2) {
            userTypeName = "corporate";
        }
        doc.write("<html><head><title>" + sessionAlertMsg + "</title>");
        doc.write("<link rel='stylesheet' type='text/css' href='./" + langId + "/" + userTypeName + "/theme/materialize.css'>");
        doc.write("<link rel='stylesheet' type='text/css' href='./" + langId + "/" + userTypeName + "/theme/layout_master.css'>");
        doc.write("<link rel='stylesheet' type='text/css' href='./" + langId + "/" + userTypeName + "/theme/application_master.css'>");
        doc.write("<link rel='stylesheet' type='text/css' href='./" + langId + "/" + userTypeName + "/theme/mediaQueryApplication.css'>");
        doc.write("<link rel='stylesheet' type='text/css' href='./" + langId + "/" + userTypeName + "/theme/materialize_master.css'>");
        //doc.write("<link rel='stylesheet' type='text/css' href='./"+langId+"/"+userTypeName+"/theme/materialize-common.css'>"); 
        doc.write("<link rel='stylesheet' type='text/css' href='./" + langId + "/" + userTypeName + "/theme/additional_temporary.css'>");
        ...
        str += "<td colspan='2' align='center' style='padding: 0px;'>";
        str += "<table width='100%' class='sessionTable'>";
        str += "<tr align='left'>";
        if (feba.domManipulator.isRTL()) {
            str += "<td align='" + left + "' colspan='2' class='sessionExpiredRowFont'><span class='sessionExpiredRow' style='padding: 10px 20px;background-position-y:0px;'></span><span>" + sessionExpiredMsg + "</span></td>";
        } else {
            str += "<td align='" + left + "' colspan='2' class='sessionExpiredRowFont'><span class='sessionExpiredRow' style='padding: 10px 20px;background-position-x:0px;'></span><span>" + sessionExpiredMsg + "</span></td>";
        }
        str += "</tr>";
        str += "<tr align='left'>";
        ...
        if (usertype != 4) {
            ...
            str += "<input type='button' value='" + goToLoginPageMsg + "' class='waves-button-input' onclick='window.close();window.opener.goToLoginPage(\"" + sessionLoginURL + "\");' style='background-color: #2196F3 !important;'></span><br /></div></td>";
            str += "</td>";
            str += "</tr>";
        } else {
            str += "<tr align='center'>";
        ...
        str += "<table width='100%'>";
        if (feba.domManipulator.isRTL()) {
            str += "<tr align='left'><td align='" + left + "' class='sessionExpiredRowFont'><span class='sessionExpiredRow' style='padding: 10px 20px;background-position-y:0px;'></span><span>" + sessionExpiringMsg + "</span><label id='secLabel' style='color: inherit;'>0</label>" + secMsg + "</td></tr>";
        } else {
            str += "<tr align='left'><td align='" + left + "' class='sessionExpiredRowFont'><span class='sessionExpiredRow' style='padding: 10px 20px;background-position-x:0px;'></span><span>" + sessionExpiringMsg + "</span><label id='secLabel' style='color: inherit;'>0</label>" + secMsg + "</td></tr>";
        }
        ...
        if (usertype != 4) {
            //str += "<tr><td><br /></td></tr>";
            str += "<tr id='rowID3' align='right'>";
            str += "<td style='padding: 0px;display:block;'><br /><div class='right rightwithmargin' ><span style='display:inline;'></span>";
            str += "<span class='waves-effect waves-light btn waves-input-wrapper' style='background-color: #2196F3 !important;'>";
            str += "<input type='button' value='" + continueSessionButton + "' class='waves-button-input' onclick='window.close();window.opener.resetSession();' style='background-color: #2196F3 !important;'> </span><br /></div></td>";
            //Added By Parvathy For UBA
            str += "<td style='padding: 0px;display:block;'><br /><div class='right rightwithmargin' ><span style='display:inline;'></span>";
            str += "<span class='waves-effect waves-light btn waves-input-wrapper' style='background-color: #2196F3 !important;top:-19px;'>";
            str += "<input type='button' value='" + Close + "' class='waves-button-input' onclick='window.close();window.opener.showParentWindow();' style='background-color: #2196F3 !important;'> </span><br /></div></td>";
            str += "</tr>";
        } else {
            str += "<tr id='rowID3' align='right'>";
            str += "<td><br /><br />";
...
function resetSession() {
    ...
    if (document.getElementById("MODAL_PREVENT_SESSION_TIMEOUT__")) {
        ...
        jQuery("[id='EXTEND_SESSION']").click();
    }
}
...
function highLightErrorRow() {
    ...
    for (var index = 0; index < totalErrorElems; index++) {
        ...
        if (jQuery(nLevelSpan).is("span")) {
            ...
            //of its parent
            var errElemId = highlightedRow.children[0].id.replace("ERR_MSG_", "");
            var errElem = feba.domManipulator.getElementById(errElemId);
            ...
            //Fix Start for replacing $FEBA_FORM_FIELD$
            var actErrMsg = jQuery(htmlBackup).find('[id="ERR_MSG_' + errElemId + '"]').text();
            if ((actErrMsg).indexOf("$FEBA_FORM_FIELD$") != -1) {
                //var modErrMsg = actErrMsg.replace("$FEBA_FORM_FIELD$",errElem.closest('P').find('.error_highlight').text().replace(':',' '));
                var modErrMsg = actErrMsg.replace("$FEBA_FORM_FIELD$", errElem.closest('P').find('span').first().text().replace(':', ''));
                htmlBackup = htmlBackup.replace(actErrMsg, modErrMsg.replace('*', ''));
            }
            //Fix End for replacing $FEBA_FORM_FIELD$
...
/**
 * added for defect 146637
 * Alert pop up did not provide an easy option to know whether an OK is clicked or cancel 
 * Hence a confirm pop up is being used
 **/
function saveOpenDownloadFileAlert(event) {
    var msg = getMessage("SaveAndOpen");
    msg = msg + " \n";
    msg = msg + getMessage("LogOutIfOpened");
    if (confirm(msg)) {
...
function restore_confirm(fieldId, event) {
    ...
    var r = confirm(getMessage("ResetMessage"));
    if (r == true) {
...
function determinePasswordStrength(password, strengthImageField, strengthValueField, groupletId, pwdType) {
    ...
    if (password.length >= rule2Length && password.length < rule3Length) {
        /*commenting the condition for call 802911*/
...
function passOverdraftStatusValuetonclick(ElementId, groupletId) {
    ...
    // Changes done for TOL 791142
    //var userTypeValue = cookieMap.get('userType');
    var userTypeValue = cookieMap['userType'];
    //if (userTypeValue==2){
    //var start = 75;
...
function toggleErrorMessage(element) {
    ...
    if (feba.domManipulator.getElement(element).first().parents('.errordisplaypulldown').attr('data-role') && feba.domManipulator.getElement(element).first().parents('.errordisplaypulldown').attr('data-role') == 'down') {
        ...
        feba.domManipulator.getElement(element).first().parents('.errordisplaypulldown').find('.arrowtoggle').attr('title', getMessage('ClickToViewLess'));
        ...
        feba.domManipulator.getElement(element).first().parents('.errordisplaypulldown').find('.arrowtoggle').attr('title', getMessage('ClickToViewMore'));
    }
}
...
function jcarouselShow(totalTemplates, groupletId) {
    ...
    setTimeout(function() {
        jQuery('.jcarousel-prev-horizontal').attr('title', getMessage('CalPrevious'));
        jQuery('.jcarousel-next-horizontal').attr('title', getMessage('Next'));
    }, 100);
...
var remUseridClose = false;

function handleRememberUserId(event) {
    remUseridClose = false;
    var rememberUserId = document.getElementById("AuthenticationFG.REMEMBER_USER_ID").checked;
    if (rememberUserId) {
        jQuery('input[name="AuthenticationFG.REMEMBER_USER_ID"]').prop("checked", false); /*added by komal.*/
        ...
        jQuery("#dialog").dialog({
            ...
            beforeClose: function(e) {
                ...
                return remUseridClose;

            }
        });
        //dialog position controlled form below function
        jQuery(".loginPage_dialog").css('left', position.left - 160);
        jQuery(".loginPage_dialog").css('top', position.top - 5);
        //dialog width for lower resolution	
        if (viewport().width < 640) {
            jQuery(".loginPage_dialog").css('width', '225px'); /* changed by komal from 118*/
            jQuery(".infopopupSeperator").css('width', '225px'); /* changed by komal from 118*/
        } else {
...
function disableRemUser() {

    jQuery('input[name="AuthenticationFG.REMEMBER_USER_ID"]').prop("checked", false);

    remUseridClose = true;
}

function remUser() {
    jQuery('input[name="AuthenticationFG.REMEMBER_USER_ID"]').prop("checked", true);
...
function handleRememberUserIdInfoClick(event) {
    ...
    if (viewport().width < 640) {
        jQuery(".loginPage_dialog").css('width', '225px');
        jQuery(".infopopupSeperator").css('width', '255px');
    } else {
        jQuery(".loginPage_dialog").css('width', '225px');
        jQuery(".infopopupSeperator").css('width', '225px');
    }
...
function getCookieMap() {
    ...
    // Added for TOL 791142 Starts
    // Map() javascript object creation is not supported above IE9, so changing map implementation as
    // array with key value pair to populate the cookie values
...
function materializeBindMultiActionsClick() {
    jQuery("[id='multiActionButtonId']").find("i").each(function() {
        jQuery(this).attr("onclick", "multiActionClick(this)");
    });
}

function multiActionClick(elementId) {
    if (jQuery(elementId).find("input").length > 0) {
        jQuery(elementId).find("input").click();
    } else {
...
function cancellogoutConfirmFunc() {
    jQuery(document).ready(function() {
...
function captchaRefresh() {
    ...
    jQuery("#TEXTIMAGE").click(function() {
