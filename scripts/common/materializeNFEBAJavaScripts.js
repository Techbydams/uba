...
var dateMaskType = null;
var hijiri_user_format = null;
var globalInst = null;
    ...
    parse: function(currentObject, data) {
        ...
        function handleNewTransaction(response, content) {
            if (response.getResponseHeader(Constants.IS_NEW_TRANSACTION)) {
                ...
                if (navigator.userAgent.match(/Trident\/7\./)) { //IE 10 and above doesnt load screen properly.
                ...
                if (isLangIDChanged()) {
                    ...
                    resetSession();
                    //For RTL changes
                    //Commenting this line for language switching issue inside application
                    //Page was not loading if it returns true from here
                    //return true;
                }
                ...
                if (jQuery.browser.msie) { //IE 10 doesnt load screen properly.
                    jQuery('link').each(function() {
                        jQuery(this).replaceWith(this.outerHTML)
                    });
