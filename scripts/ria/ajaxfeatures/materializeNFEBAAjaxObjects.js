    ...
    getOptions: function(options, ajaxParam) {
        ...
        return feba.domManipulator.extendObject({
            ...
            beforeSend: function(xhr) {
                ...
                xhr.setRequestHeader('finParam', finParam);
...
feba.domManipulator.createChildClass(feba.js.ajax.base, "feba.js.ajax.grouplet", {}, {
    ...
    init: function(options) {
        ...
        */
        ...
        var leftWidgets = false;
        var welcomeWidget = false;
        var mailsWidget = false;
        var notificationWidget = false;

        if (this.options.target) {
            if (jQuery(".left-container") != undefined && jQuery(".left-container") != "") {
                if (jQuery(".left-container").find(jQuery("#" + this.options.target)).length > 0) {
                    leftWidgets = true;
                }
            }
            if (jQuery(".widget.container-xtrasmall_ux5") != undefined && jQuery(".widget.container-xtrasmall_ux5") != "") {
                if (jQuery(".widget.container-xtrasmall_ux5").find(jQuery("#" + this.options.target)).length > 0) {
                    welcomeWidget = true;
                }
            }
            if (!(leftWidgets || welcomeWidget)) {
                if (jQuery(".mailsBox .dynamicDataContentHolderDiv") != undefined && jQuery(".mailsBox .dynamicDataContentHolderDiv") != "") {
                    if (jQuery(".mailsBox .dynamicDataContentHolderDiv").find(jQuery("#" + this.options.target)).length > 0) {
                        mailsWidget = true;

                    }
                }
                if (jQuery(".notifyBox .dynamicDataContentHolderDiv") != undefined && jQuery(".notifyBox .dynamicDataContentHolderDiv") != "") {
                    if (jQuery(".notifyBox .dynamicDataContentHolderDiv").find(jQuery("#" + this.options.target)).length > 0) {
                        notificationWidget = true;
                    }
                }
                if (!(mailsWidget || notificationWidget)) {
    ...
    onComplete: function() {
        ...
        if (userType != '4') {
            feba.domManipulator.documentReady(rowSelectionOnCheckBoxClick());
            feba.domManipulator.documentReady(materializeAutoComplete());
            feba.domManipulator.documentReady(selectPayeeOnQuickPayWidget());
            feba.domManipulator.documentReady(materializeRadioAndCheckBox());
            feba.domManipulator.documentReady(materializeMultiSelect());
            feba.domManipulator.documentReady(jQuery('select').material_select());
            /*feba.domManipulator.documentReady(convertCheckBoxes());
            feba.domManipulator.documentReady(convertRadioButton());
            feba.domManipulator.documentReady(convertComboBox());*/
            ...
            feba.domManipulator.documentReady(materializeDelink());
            feba.domManipulator.documentReady(materializeTextAreaInitialization());
            feba.domManipulator.documentReady(collapsibleOnload());
            feba.domManipulator.documentReady(materializeMegamenuEffects());
            feba.domManipulator.documentReady(materializeTabEffects());
            feba.domManipulator.documentReady(materializeVerticalTabEffects());
            feba.domManipulator.documentReady(materializeBindMultiActionsClick());
            ...
            if (jQuery(".printwrapper").length > 0) {
                feba.domManipulator.documentReady(expandRowDetailInPrintMode());
            }
            if (jQuery(".printwrapper").length > 0) {
        ...
        if (isMasterGrouplet && isMasterGrouplet != null && isMasterGrouplet.length > 0 && isMasterGrouplet == "Y") {
            ...
            //var htmlForMasterGrouplet="<a  href='#' class=\"breadcrumb\">"+titleAttr+"</a>";	

            //jQuery(jQuery('.breadcrumbnew1')[jQuery('.breadcrumbnew1').length-1]).append(htmlForMasterGrouplet);
            if (arguments[0] && arguments[0].status == '200') {
                //jQuery(htmlForMasterGrouplet).insertAfter(jQuery('.breadcrumb')[jQuery('.breadcrumb').length-1]);
                /*if(jQuery('.breadcrumb').length =='0')
                {
                jQuery(jQuery('.breadcrumb')[jQuery('.breadcrumb').length-1]).append(htmlForMasterGrouplet)
                }*/
                ...
                /*adding the below line for placing encoded HTML value*/
                jQuery('title').html(titleMessage);
            }
                    ...
                    for (i = 0; i < noElements; i++) {
                        ...
                        if (jQuery(currElement).children('.widget').length > 0 && parseInt(vpWidth) <= 900) {
                            ...
                            if (!jQuery(currElement).hasClass('height-resized')) {
                                ...
                                /*Commeneted for Lumi for for Height Issue */
            ...
            //jQuery(".customRakLeftWrapper").removeClass('hideElement');
        } else {
            //do nothing.
        }
        try {
        ...
        /* STARTS: fetch all dynamicContentHolderDiv and loop through the same and hide it if container is already visible on load.
         *  The container will get displayed only when user clicks on the container*/
        for (divId = 0; divId < jQuery('.dynamicDataContentHolderDiv').length; divId++) {
            var divObj = jQuery('.dynamicDataContentHolderDiv')[divId];
            var currentDivId = jQuery(divObj).attr('id');
            var containerDiv = currentDivId + "_Container";
            if (jQuery(containerDiv).css('display') != 'undefined' && jQuery(containerDiv).css('display') == 'block') {
                jQuery('.' + containerDiv).toggle();
            }
        }
        /* ENDS dynamic content painting*/

        /*var displayDiv = jQuery('.dynamicDataContentHolderDiv');
		var animationDivId="";
		for(divId=0;divId<jQuery('.dynamicDataContentHolderDiv').length;divId++){		
			var currDivId = jQuery('.dynamicDataContentHolderDiv')[divId];
			var currDivId= jQuery(currDivId).attr('id');
			
			if(jQuery('#'+currDivId).find('#'+this.options.target).length>0){
				animationDivId=currDivId;
		 		//jQuery('#'+currDivId).siblings().unblock();
		 		//var elemWidth=jQuery('#'+currDivId).siblings().css('width');
 				//jQuery('#'+currDivId).css('left',elemWidth);
 				if(!jQuery('#'+currDivId).is(':visible')){
 		 			if(!(jQuery('.dynamicDataContentHolderDiv:visible').attr('id')==currDivId)){
 						jQuery('.dynamicDataContentHolderDiv:visible').toggle( "slide" );
 					}
			 		jQuery('#'+animationDivId).toggle( "slide" ,function(){
	 					var positionElem = jQuery('#'+animationDivId).prev('li').position().top;
	 					jQuery('#'+animationDivId).css('left',elemWidth);
	 					jQuery('#'+animationDivId).css('top',positionElem+'px');
		   				setTimeout(function(){
		         			jQuery('#'+animationDivId).css('top',positionElem+'px');
						},200);
	        		});
    ...
    closeGrouplet: function() {
        ...
        if (result) {
            ...
            try {
                jQuery("#" + this.options.target).getNiceScroll().remove();
            } catch (e) {
                console.log("Error while removing scroll" + e.message);
                console.log("Error while removing scroll" + e.stack);
            }
            ...
            if ((this.options.removeUrl)) {
                ...
                var rpc = new feba.js.ajax.rpcRequest({
                    ...
                    doNotShowBlocker: this.options.doNotShowBlocker,
                    executeOnLoad: false,
                    onFailure: function() {
            ...
            if (feba.js.ajax.groupletCounter === 0) {
                ...
                if (feba.js.common.configurable_Widget_Present == 'N') {
                    ...
                    errorMessage_closingAllWidgets = getMessage("NoWidgetsUX5");
                }
                //Added for RTL issue for error message
                var direction = "ltr";
                var encodeChar = "";
                ...
                if (feba.domManipulator.getElementById('display_error_var').attr("value") == "A") {
                    LIB.__HANDLE_ERROR__(null, "<div id=\"widLibWrapper\" role = \"//alert\" class=\"redbg errorMsgPositionAlign\"><a id=\"errorlink1\" href=\"#\"><img class=\"absmiddle\" title=\"" + getMessage("NoWidgetsTitle") + "\" " +
                        "alt=\"" + getMessage("NoWidgetsAlt") + "\" src=\"" + getMessage("NoWidgetsImageSrc") + "\"></a><span style=\"width:80%; display:inline-table;\" dir=\"" + direction + "\"><p style=\"display: inline\" > " + encodeChar + "[CONTLS0004" + encodeChar + "] </p><p style=\"display: inline\" >" + encodeChar + "[110817" + encodeChar + "] </p><p style=\"display: inline\" > " + errorMessage_closingAllWidgets + "</p></span></div>", null, null, "true", true);
                } else if (feba.domManipulator.getElementById('display_error_var').attr("value") == "E") {
                    LIB.__HANDLE_ERROR__(null, "<div id=\"widLibWrapper\" role = \"//alert\" class=\"redbg errorMsgPositionAlign\"><a id=\"errorlink1\" href=\"#\"><img class=\"absmiddle\" title=\"" + getMessage("NoWidgetsTitle") + "\" " +
                        "alt=\"" + getMessage("NoWidgetsAlt") + "\" src=\"" + getMessage("NoWidgetsImageSrc") + "\"></a><span style=\"width:80%; display:inline-table;\" dir=\"" + direction + "\"><p style=\"display: none\" > " + encodeChar + "[CONTLS0004" + encodeChar + "] </p><p style=\"display: inline\" > " + encodeChar + "[110817" + encodeChar + "] </p><p style=\"display: inline\" > " + errorMessage_closingAllWidgets + "</p></span></div>", null, null, "true", true);
                } else if (feba.domManipulator.getElementById('display_error_var').attr("value") == "I") {
                    LIB.__HANDLE_ERROR__(null, "<div id=\"widLibWrapper\" role = \"//alert\" class=\"redbg errorMsgPositionAlign\"><a id=\"errorlink1\" href=\"#\"><img class=\"absmiddle\" title=\"" + getMessage("NoWidgetsTitle") + "\" " +
                        "alt=\"" + getMessage("NoWidgetsAlt") + "\" src=\"" + getMessage("NoWidgetsImageSrc") + "\"></a><span style=\"width:80%; display:inline-table;\" dir=\"" + direction + "\"><p style=\"display: inline\" > " + encodeChar + "[CONTLS0004" + encodeChar + "] </p><p style=\"display: none\" > " + encodeChar + "[110817" + encodeChar + "] </p><p style=\"display: inline\" > " + errorMessage_closingAllWidgets + "</p></span></div>", null, null, "true", true);
                } else {
                    LIB.__HANDLE_ERROR__(null, "<div id=\"widLibWrapper\" role = \"//alert\" class=\"redbg errorMsgPositionAlign\"><a id=\"errorlink1\" href=\"#\"><img class=\"absmiddle\" title=\"" + getMessage("NoWidgetsTitle") + "\" " +
                        "alt=\"" + getMessage("NoWidgetsAlt") + "\" src=\"" + getMessage("NoWidgetsImageSrc") + "\"></a><span style=\"width:80%; display:inline-table;\" dir=\"" + direction + "\"><p style=\"display: none\" > " + encodeChar + "[CONTLS0004" + encodeChar + "] </p><p style=\"display: none\" > " + encodeChar + "[110817" + encodeChar + "] </p><p style=\"display: inline\" > " + errorMessage_closingAllWidgets + "</p></span></div>", null, null, "true", true);
                }
            }
        } else {
            feba.js.ajax.groupletCounter = feba.js.ajax.groupletCounter + 1;
...
feba.domManipulator.createChildClass(feba.js.ajax.base, "feba.js.ajax.rpcRequest", {}, {
    ...
    execute: function() {
        ...
        if (this.options && this.options.doNotShowBlocker && this.options.doNotShowBlocker == "Y") {
            // no blocker
        } else {
...
feba.domManipulator.createChildClass(feba.js.ajax.base, "feba.js.ajax.updateField", {}, {
    ...
    setListeners: function() {
        ...
        if (eventElement.length !== 0) {
            ...
            if (feba.domManipulator.hasValue(eventElement) && (LIB.__GET_DOM__(source).type != Constants.RADIO || LIB.__GET_DOM__(source).checked == true)) {
                ...
                //added for Mobile Login to prevent ajax call on Login/Transaction page LOAD .
                if (this.options.forcePreventAjaxCall && this.options.forcePreventAjaxCall != "") {
                    this.options.executeOnLoad = false;
                    return;
    ...
    handler: function() {
        ...
        if (target != undefined && target.length != 0 && ((target.indexOf(",") != -1) || (target.indexOf("=") != -1))) {
            targetArray = target.split(",");
            var targetLen = targetArray.length;
            for (var i = 0; i < targetLen; i++) {
                if (targetArray[i].indexOf("=") != -1) {
            ...
            if (firstTarget.length === 0) {
                ...
                if (options.forcePreventAjaxCall && options.forcePreventAjaxCall != "") {
...
feba.js.ajax.base.extend("feba.js.ajax.modalView", {}, {
     ...
     */
    execute: function() {
        closeAllAutoCompleteOptionsLink();
        ...
        if (containerDiv.length == 0 && this.options.isModalCloseEvent != "Y") {
            ...
            if (document.getElementById("NLP_SEARCH__")) {
                jQuery("[id='NLP_SEARCH__']").addClass("disablePointerEvents");
            }
        }
        this.showModalWithProcessing();
        LOG.logMessages("In event handler Execute method,raising the request ");

        /*indrajit:Resetting global values on modal window opening*/
                                ...
                                // FIX for population of Dropdown: EBUX3 START
                                    ...
                                    for (var k = 0; k < optionsLength; k++) {
                                        if (target.options[k].value == this.content[j][1]) {
                                            ...
                                            //Fix for populating the values in the UX5 dropdown
                                            var dropdownText = jQuery(target).parent().siblings('input.autocomplete-dropdown');
                                            jQuery(dropdownText).val(jQuery(target).find('option:selected').text());
                                        }
                                    }
                                }
    ...
    handler: function() {
        ...
        if (containerDiv.length == 0 && (actionInHeader == null || actionInHeader == undefined)) {
            ...
            if (actionInHeader == "REFRESH") {
                ...
                var mailComposePageTitle = "Compose Message";
                var UIType = jQuery("#UIType").val();
                var action = feba.features["MODAL_VIEW_CONTAINER"].options.actionElement;
                var isMailComposeTilte = title.indexOf(mailComposePageTitle) >= 0;
                if (UIType == "UX5" && isMailComposeTilte) {
                    if (action == "TEMPLATE_CANCEL") {
                        modalDialogObj.dialog("close");
                        return;
                    } else if (action == "TEMPLATE_SELECT") {
                        //jQuery("[title=Compose]")[0].click();
                        var params = feba.features["MODAL_VIEW_CONTAINER"].options.groupletParameters.split("&");
                        var message = "";
                        var subject = "";
                        for (var i = 0; i < params.length; i++) {
                            if (params[i].indexOf("autocomplete-dropdown=") >= 0) {
                                subject = params[i].replace('autocomplete-dropdown=', '');
                                subject = decodeURIComponent(subject);
                            }
                            if (params[i].indexOf("SubjectDropDown=") >= 0) {
                                //message = params[i].split("=")[1];
                                message = params[i].replace('SubjectDropDown=', '');
                                message = decodeURIComponent(message);
                            }

                        }
                        modalDialogObj.dialog("close");
                        document.getElementById("MailFG.SUBJECT").value = subject;
                        document.getElementById("MailFG.MAIL_BODY").value = message;
                        return;
                    }
                }
            }
            ...
            if (this.isFirstExecution) {
                ...
                if (businessError && (parentViewId == "true" || (this.paintExceptionOnPage == "true") || (this.paintExceptionOnPage == true))) {
                    ...
                    /*Added as part of Page focus issue when modal window abruptly closes due to business error*/
                    ...
                    if (isDashboardScreen) {
                        ...
                        var target = dashLinkEvent.target;
                        var widgetElem = jQuery(target).parents('[class*="widget-body"]').children();
                        if (typeof widgetElem != typeof undefined && widgetElem != null && widgetElem.length != 0) {
                            var widgetPos = jQuery(widgetElem[0]).position().top;
        ...
        var title = decodeURIComponent(this.parser.title);
        var modalDialogObj = this.modalDialog ? this.modalDialog : containerDiv.parent();
        ...
        if (!(!(this.source == feba.features.MODAL_VIEW_CONTAINER.options.source) && (this.source == "dummy" && feba.features.MODAL_VIEW_CONTAINER.options.abortEvent == "CANCEL_SPLASH_PAGE_EVENT__"))) {
            ...
            if (!(this.abortEvent == undefined || this.abortEvent == null)) {
                ...
                if (this.hideTitle == Constants.TRUE) {
                    ...
                    /*var titleBar= jQuery(".ui-dialog-titlebar");
	         			...
	         			if((typeof(titleBar)!=null && typeof(titleBar)!=undefined) && (typeof(title)!=null && typeof(title)!=undefined))
                        {
                        var marginleftPx= (titleBar.width()/2-title.width()/2)+"px";
                        feba.domManipulator.getElement(".ui-dialog-title").css('margin-left',marginleftPx); 
                        }*/


                    feba.domManipulator.getElement(".ui-dialog-title").addClass('uiconfirmtitle');

                    /*Added for Showing the Overlay Title in Middle -Start*/
                    /* commented for overlay center alignment*/
                    // feba.domManipulator.getElement(".ui-dialog-title").addClass('ui-dialog-heading1-alignment');
                ...
                if (!this.appendDefaultMinHeightToModal || (this.appendDefaultMinHeightToModal && !(this.appendDefaultMinHeightToModal == "false"))) {
            ...
            if (titleBar.length > 1) {
                var titleBarDash = jQuery(titleBar[1]);
                var titleDash = jQuery(title[1])
                if (titleBarDash != null && typeof titleBarDash != typeof undefined && titleDash != null && typeof titleDash != typeof undefined) {
                    var titleBarWidth = jQuery(titleBarDash).width();
                    var titleWidth = jQuery(titleDash).width();
                    var marginleftPx = (titleBarWidth / 2 - titleWidth / 2) + "px";
                    jQuery(titleDash).css('margin-' + left + '', marginleftPx);
                }
            } else {
                if ((typeof(titleBar) != null && typeof(titleBar) != undefined) && (typeof(title) != null && typeof(title) != undefined)) {
                    var marginleftPx = (titleBar.width() / 2 - title.width() / 2) + "px";
                    jQuery(".ui-dialog-title").css('margin-' + left + '', marginleftPx);
                }
            }
            ...
            /*added code for Removing the Close icon --Start */
            if (this.source == "CorporateUserDashboardUX5_INWD__1:InstaWidgetsDashBoard") {
                feba.domManipulator.getElement(".ui-dialog-titlebar-close").hide();
                jQuery('.ui-button').css('display', 'none');
            }
            /*added code for Removing the Close icon --End */
            /* commented for overlay center alignment*/
            /*feba.domManipulator.getElement(".ui-dialog-title").addClass('ui-dialog-heading1-alignment');*/
            /*Added for Showing the Overlay Title in Middle -End*/
            if (feba.domManipulator.getElementById("HORIZONTAl_OVERLAY")) {

                var isHorizontalOverlay = feba.domManipulator.getElementById("HORIZONTAl_OVERLAY").val();
                if (isHorizontalOverlay == 'true') {
                    feba.domManipulator.getElement(".ui-dialog-title").addClass('overlayHeadingStyle');
                    feba.domManipulator.getElement(".ui-dialog-titlebar").addClass('overlayTitleHeight');
...
function setTarget(target, content, selectValue, targetContent, isTargetAnImage) {
    ...
    switch (targetType) {
        case Constants.SIMPLE_SELECT:
            ...
            /*Modified for lumi for Updating the dropdown--Start*/

            refreshAutoCompleteValues(target);
            /*Modified for lumi for Updating the dropdown--End*/
...
// function to hide auto complete dropdowns fix for ticket #769278
function closeAllAutoCompleteOptionsLink() {
